...
export async function fetchLiveSerp(params: {
  ...
  const apiKey =
    ...
    HARDCODED_SERP_API_KEY;

  if (
    !apiKey ||
    apiKey.trim() === '' ||
    apiKey === 'ADD_YOUR_SERP_API_KEY_HERE' ||
    apiKey === 'MY_SERPAPI_KEY'
  ) {
    const err: any = new Error(
      'SERP API key is missing in backend. Please set HARDCODED_SERP_API_KEY = "YOUR_KEY" in server/serp.ts or set SERPAPI_KEY in environment variables.'
    );
    err.code = 'NO_API_KEY';
    throw err;
  }
