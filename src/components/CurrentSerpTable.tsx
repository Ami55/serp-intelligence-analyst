...
import { ExternalLink, Database, Globe, Smartphone, Monitor, Layers, Tag } from 'lucide-react';
import { SERPSnapshot } from '../types';
import { ensurePageType, getPageTypeBadgeStyle, getPageTypeDistribution } from '../utils/pageType';
...
export const CurrentSerpTable: React.FC<CurrentSerpTableProps> = ({ snapshot }) => {
  ...
  const distribution = getPageTypeDistribution(snapshot.organicResults);
  ...
  return (
    <div id="current-serp-section" className="bg-slate-900 rounded-xl border border-slate-800 shadow-xs mb-8 overflow-hidden">
      ...
      <div className="p-5 border-b border-slate-800 bg-slate-900/60">
        ...
        {/* Page Type Landscape Breakdown */}
        {distribution.length > 0 && (
          <div className="mt-4 pt-3 border-t border-slate-800/80">
            <div className="flex items-center space-x-2 text-xs font-semibold text-slate-300 mb-2">
              <Layers className="w-3.5 h-3.5 text-indigo-400" />
              <span>Page Type Landscape Mix:</span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {distribution.map((dist, idx) => {
                const style = getPageTypeBadgeStyle(dist.pageType);
                return (
                  <span
                    key={idx}
                    className={`inline-flex items-center space-x-1 px-2.5 py-1 rounded-md text-xs font-medium border ${style}`}
                  >
                    <span>{dist.pageType}:</span>
                    <strong className="font-bold">{dist.count}</strong>
                    <span className="opacity-75">({dist.percentage}%)</span>
                  </span>
                );
              })}
            </div>
        ...
        <table className="w-full text-left text-xs text-slate-300">
          <thead className="bg-slate-950 text-slate-300 font-semibold border-b border-slate-800 uppercase tracking-wider">
            <tr>
              ...
              <th scope="col" className="py-3 px-4 w-44">Page Type</th>
          ...
          <tbody className="divide-y divide-slate-800/80 bg-slate-900">
            {snapshot.organicResults.map((res) => {
              const pageType = ensurePageType(res);
              const badgeStyle = getPageTypeBadgeStyle(pageType);
              ...
              return (
                <tr key={`${res.position}-${res.domain}`} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <span className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded text-[11px] font-semibold border whitespace-nowrap ${badgeStyle}`}>
